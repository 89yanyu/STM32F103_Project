...
u8 USART_SendBuff[USART_BUFFLEN];
u8 USART_RecvBuff[USART_BUFFLEN];
u32 USART_SendHead, USART_SendTail, USART_RecvLen;
u8 USART_isSending;
u16 USART_Head, USART_CRC;
USART_CONTROL_TYP USART_Operate = USART_CONTROL_MAX;
u16 USART_WaitLen;
...
void usart_ProcOneChar(u8 nowchar)
{
...
{
    if ((USART_State != USART_STATE_READY) || (USART_Operate == USART_CONTROL_MAX))
    {
        return 0;
    }
    *op = USART_Operate;
