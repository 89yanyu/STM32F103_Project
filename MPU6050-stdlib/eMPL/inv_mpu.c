...
#define i2c_write(a,b,c,d)   MPU6050_WriteBuff(a,b,c,d)
#define i2c_read(a,b,c,d)    MPU6050_ReadBuff(a,b,c,d)
#define delay_ms(x)          Delay_ms_Lazy(x)
#define get_ms(x)            mget_ms(x)
//static inline int reg_int_cb(struct int_param_s *int_param)
//{
//    return 0;
//}
#define log_i  UIO_printf
#define log_e  UIO_printf
    ...
    for (ii = 0; ii < length; ii += this_write) {
        ...
        if (memcmp(firmware+ii, cur, this_write))
        {
            for (jj = 0; jj < this_write; jj++)
                log_e("%d: 0x%02x 0x%02x\r\n", jj, firmware[ii + jj], cur[jj]);
            return 5;
        }
    }
