...
u8 MPU6050_ReadBuff(u8 Addr, u8 Reg, u8 Len, u8 *Buff)
{
    I2C_Start();
...
u8 MPU6050_WriteBuff(u8 Addr, u8 Reg, u8 Len, u8 *Buff)
{
    u8 i;
    I2C_Start();
    I2C_WriteByte((Addr << 1) | 0);
...
u8 MPU6050_GetTemperature(short *tp)
{
    ...
    if (0 == MPU6050_ReadBuff(MPU_ADDR, MPU6050_TEMPOUTHREG, 2, buff))
    {
...
u8 MPU6050_GetGyroscope(short *gx, short *gy, short *gz)
{
    ...
    if (0 == MPU6050_ReadBuff(MPU_ADDR, MPU6050_GYROXOUTHREG, 6, buff))
    {
...
u8 MPU6050_GetAccelerometer(short *ax,short *ay,short *az)
{
    ...
    if (0 == MPU6050_ReadBuff(MPU_ADDR, MPU6050_ACCELXOUTHREG, 6, buff))
    {
...
u8 MPU6050_GetAllData(short *ax, short *ay, short *az, short *tp, short *gx, short *gy, short *gz)
{
    ...
    if (0 == MPU6050_ReadBuff(MPU_ADDR, MPU6050_ACCELXOUTHREG, 14, buff))
    {
        *ax=((u16)buff[ 0] << 8) | buff[ 1];
        *ay=((u16)buff[ 2] << 8) | buff[ 3];
        *az=((u16)buff[ 4] << 8) | buff[ 5];
