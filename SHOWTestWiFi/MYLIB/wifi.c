...
char WIFI_SRVIP[32] = {0};//

int WIFI_SRVPORT = 23333;
...
void wifi_SendCfg(char *Cfg)
{
    ...
    if (sending == 0)
    {
        ...
        //UIO_printf("Send begin. %d -> %d, state: %d\r\n", sendhead, sendtail, sending);
    }
}
...
{
    if ((isclosed == 0) && (len == 6) && (memcmp(recv, "CLOSED", len) == 0))
    {
        isclosed = 1;
        UIO_printf("Socket closed.\r\n");
        return;
    }
    if (0 == waitingmsg)
    {
